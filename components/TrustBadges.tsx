...
const Badge = ({ icon: Icon, text }: { icon: any, text: string }) => (
  <div className="flex items-center gap-3 px-6 py-4 bg-card backdrop-blur-md rounded-2xl border border-border shadow-[0_8px_22px_rgba(15,23,42,0.05)] hover:border-emerald-200 transition-all">
    <div className="p-2.5 bg-green-50 rounded-lg">
      <Icon className="w-5 h-5 sage-text" />
    </div>
    <span className="text-sm font-semibold text-foreground/90 tracking-tight whitespace-nowrap">{text}</span>
  </div>
);

const TrustBadges: React.FC = () => {
  return (
    <section className="py-14 bg-background border-y border-slate-800">
