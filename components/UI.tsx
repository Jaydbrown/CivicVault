...
export const StatusChip: React.FC<{ status: string; className?: string }> = ({ status, className = '' }) => {
  ...
  const tone =
    normalized === 'proposed'  ? 'warning'  :
    normalized === 'active'    ? 'info'     :
    normalized === 'completed' ? 'success'  :
    normalized === 'ended'     ? 'success'  :
    normalized === 'incomplete'? 'error'    :
                                 'neutral';
...
export const MetricCard: React.FC<{
  ...
  className?: string;
}> = ({ label, value, sublabel, icon, accent, className = '' }) => {
  const accentBg = accent === 'emerald' ? 'bg-emerald-50' :
