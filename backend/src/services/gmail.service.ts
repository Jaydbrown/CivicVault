...
export class GmailService {
  ...
  async sendEmail(userEmail: string, refreshToken: string, subject: string, html: string) {
    ...
    const info = await transporter.sendMail({
      from: `CivicVault Chat <${userEmail}>`,
      to: userEmail,
      subject,
      html,
    });
  ...
  /**
   * Production path: send FROM the CivicVault notification mailbox (OAuth refresh token or app password).
   */
