...
const CIVIC_VAULT_ABI = [
  ...
  },
  {
    ...
    inputs: [
      ...
      { name: "kycProofHashes", type: "bytes32[]" },
    ],
    outputs: [],
  },
  {
    type: "function",
    name: "updateMemberKYCHash",
    stateMutability: "nonpayable",
    inputs: [
      { name: "wallet", type: "address" },
  ...
  {
    type: "function",
    name: "updateInvestmentDocuments",
    stateMutability: "nonpayable",
    inputs: [
      { name: "investmentId", type: "uint256" },
  ...
  {
    type: "event",
    name: "YieldDepositExecuted",
    anonymous: false,
    inputs: [
      { indexed: true, name: "proposalId", type: "uint256" },
      { indexed: true, name: "investmentId", type: "uint256" },
      { indexed: false, name: "amount", type: "uint256" },
      { indexed: false, name: "expenseReportCID", type: "string" },
      { indexed: false, name: "timestamp", type: "uint256" },
    ],
  },
] as const;
...
export async function fetchDaoParticipants(daoAddress: Address): Promise<DaoParticipant[]> {
  const [creator, members] = await Promise.all([
    readContractRpc<Address>({
      address: daoAddress,
      abi: CIVIC_VAULT_ABI,
      functionName: "creator",
    }),
    readContractRpc<readonly Address[]>({
      address: daoAddress,
      abi: CIVIC_VAULT_ABI,
  ...
  // Use getAdmins() as primary source — no log scanning needed.
  let adminList: Address[] = [];
  try {
    adminList = [
      ...(await readContractRpc<readonly Address[]>({
        address: daoAddress,
    ...
    // Fall back to log scanning for older deployments.
    try {
      const [added, removed] = await Promise.all([
        getLogsWithRpcFallback({
          address: daoAddress,
          event: { type: "event", name: "AdminAdded", inputs: [{ indexed: true, name: "admin", type: "address" }] },
          fromBlock: 0n,
          toBlock: "latest",
        }),
        getLogsWithRpcFallback({
          address: daoAddress,
          event: { type: "event", name: "AdminRemoved", inputs: [{ indexed: true, name: "admin", type: "address" }] },
          fromBlock: 0n,
          toBlock: "latest",
        }),
      ]);
      const adminSet = new Set<Address>();
      for (const log of added as unknown as Array<{ args: { admin: Address } }>) adminSet.add(log.args.admin);
      for (const log of removed as unknown as Array<{ args: { admin: Address } }>) adminSet.delete(log.args.admin);
      adminList = [...adminSet];
    } catch { /* keep empty */ }
  }

  // Finance managers still require log scanning (no getFinanceManagers() on-chain).
  let fmSet = new Set<Address>();
  try {
    const [fmAdd, fmRem] = await Promise.all([
      getLogsWithRpcFallback({
        address: daoAddress,
        event: { type: "event", name: "FinanceManagerAdded", inputs: [{ indexed: true, name: "manager", type: "address" }] },
        fromBlock: 0n,
        toBlock: "latest",
      }),
      getLogsWithRpcFallback({
        address: daoAddress,
        event: { type: "event", name: "FinanceManagerRemoved", inputs: [{ indexed: true, name: "manager", type: "address" }] },
        fromBlock: 0n,
        toBlock: "latest",
      }),
    ]);
    for (const log of fmAdd as unknown as Array<{ args: { manager: Address } }>) fmSet.add(log.args.manager);
    for (const log of fmRem as unknown as Array<{ args: { manager: Address } }>) fmSet.delete(log.args.manager);
  } catch { /* keep empty */ }
...
export async function depositInvestmentYield(
  ...
  const { request } = await simulateContractRpc({
    address: params.daoAddress,
    abi: CIVIC_VAULT_ABI,
    functionName: "proposeYieldDeposit",
...
export async function addMemberToDao(
  ...
  const proofReferenceUsed =
    ...
    `manual://civicvault/${params.memberWallet.toLowerCase()}/${Date.now()}`;
...
export async function proposeYieldDepositOnDao(
  wallet: PrivyEthereumWallet,
  params: {
    daoAddress: Address;
    investmentId: number;
  ...
  const { request, result } = await simulateContractRpc({
    address: params.daoAddress,
    abi: CIVIC_VAULT_ABI,
    functionName: "proposeYieldDeposit",
    args: [BigInt(params.investmentId), amountRaw, params.expenseReportCID],
  ...
  const txHash = await walletClient.writeContract(request as never);
  await publicClient.waitForTransactionReceipt({ hash: txHash });
  return { txHash, proposalId: result as bigint };
}

export async function approveYieldDepositOnDao(
  wallet: PrivyEthereumWallet,
  daoAddress: Address,
  proposalId: number
): Promise<Hex> {
  ...
  const { request } = await simulateContractRpc({
    address: daoAddress,
    abi: CIVIC_VAULT_ABI,
    functionName: "approveYieldDeposit",
...
export async function executeYieldDepositOnDao(
  wallet: PrivyEthereumWallet,
  params: {
    daoAddress: Address;
    proposalId: number;
  ...
  // Read the proposal to determine how much USDC the proposer needs to approve.
  const proposal = await readContractRpc({
    address: params.daoAddress,
    abi: CIVIC_VAULT_ABI,
    functionName: "getYieldProposal",
    args: [BigInt(params.proposalId)],
  }) as { proposer: Address; amount: bigint; executed: boolean; approvals: bigint };

  if (proposal.executed) throw new Error("Yield proposal already executed.");
  if (Number(proposal.approvals) < 3) throw new Error(`Need 3 approvals — currently ${proposal.approvals}.`);

  // Ensure the proposer (who funds the deposit) has granted sufficient allowance.
  const allowance = await readContractRpc({
    address: USDC_ADDRESS,
    abi: ERC20_ABI,
    functionName: "allowance",
    args: [proposal.proposer, params.daoAddress],
  }) as bigint;

  if (allowance < proposal.amount) {
    throw new Error(
      `Proposer (${proposal.proposer}) must approve ${formatUsdcAmount(proposal.amount)} USDC to the DAO before execution.`
    );
  }
  ...
  const { request } = await simulateContractRpc({
    address: params.daoAddress,
    abi: CIVIC_VAULT_ABI,
    functionName: "executeYieldDeposit",
    args: [BigInt(params.proposalId)],
    account,
  });
...
export async function fetchYieldProposal(
  daoAddress: Address,
  proposalId: number
): Promise<YieldProposal> {
  const raw = await readContractRpc({
    address: daoAddress,
    abi: CIVIC_VAULT_ABI,
    functionName: "getYieldProposal",
    args: [BigInt(proposalId)],
  }) as {
    id: bigint; investmentId: bigint; amount: bigint; expenseReportCID: string;
    proposer: Address; approvals: bigint; executed: boolean; createdAt: bigint;
  };
  return {
    id: Number(raw.id),
    investmentId: Number(raw.investmentId),
    amount: raw.amount,
    expenseReportCID: raw.expenseReportCID,
...
export async function fetchProposalApprovals(
  daoAddress: Address,
  proposalId: number,
  ...
  const results = await Promise.all(
    adminAddresses.map(async (admin) => {
      const approved = await readContractRpc<boolean>({
        address: daoAddress,
        abi: CIVIC_VAULT_ABI,
        functionName: "hasApprovedProposal",
        args: [BigInt(proposalId), admin],
      });
      return [admin.toLowerCase(), approved] as const;
    })
  );
...
export async function releaseNextPhaseOnDao(
  wallet: PrivyEthereumWallet,
  params: {
    daoAddress: Address;
    investmentId: number;
  ...
  const { request } = await simulateContractRpc({
    address: params.daoAddress,
    abi: CIVIC_VAULT_ABI,
    functionName: "releaseNextPhase",
    args: [BigInt(params.investmentId), params.recipient ?? ("0x0000000000000000000000000000000000000000" as Address)],
    account,
  });
...
export async function batchAddMembersToDao(
  wallet: PrivyEthereumWallet,
  params: {
    daoAddress: Address;
    entries: Array<{ wallet: Address; proofReference?: string }>;
  }
): Promise<Hex> {
  await wallet.switchChain(APP_CHAIN_ID);
  const walletClient = await getWalletClient(wallet);
  ...
  const wallets = params.entries.map((e) => e.wallet);
  const hashes = params.entries.map((e) =>
    deriveKycProofHash(
      e.proofReference?.trim() ||
        `manual://civicvault/${e.wallet.toLowerCase()}/${Date.now()}`
    )
  );
...
export async function updateMemberKycHashOnDao(
  wallet: PrivyEthereumWallet,
  params: {
    daoAddress: Address;
    memberWallet: Address;
  ...
  const walletClient = await getWalletClient(wallet);
  const account = wallet.address as Address;
  const newHash = deriveKycProofHash(params.proofReference);

  const { request } = await simulateContractRpc({
    address: params.daoAddress,
    abi: CIVIC_VAULT_ABI,
    functionName: "updateMemberKYCHash",
    args: [params.memberWallet, newHash],
    account,
  });
...
// ===== INVESTMENT DOCUMENT UPDATE =====

export async function updateInvestmentDocumentsOnDao(
  wallet: PrivyEthereumWallet,
  params: {
    daoAddress: Address;
    investmentId: number;
  ...
  const { request } = await simulateContractRpc({
    address: params.daoAddress,
    abi: CIVIC_VAULT_ABI,
    functionName: "updateInvestmentDocuments",
    args: [BigInt(params.investmentId), params.documentCIDs],
    account,
  });
...
export async function fetchInvestmentsByStatus(
  daoAddress: Address,
  status: 0 | 1 | 2 | 3
): Promise<number[]> {
  const ids = await readContractRpc<readonly bigint[]>({
    address: daoAddress,
    abi: CIVIC_VAULT_ABI,
    functionName: "getInvestmentsByStatus",
    args: [status],
  });
...
export async function fetchTotalDaoCount(): Promise<number> {
  const count = await readContractRpc<bigint>({
    address: FACTORY_ADDRESS,
    abi: FACTORY_ABI,
    functionName: "totalDAOCount",
  });
  return Number(count);
}

export async function fetchIsValidDao(daoAddress: Address): Promise<boolean> {
  return readContractRpc<boolean>({
    address: FACTORY_ADDRESS,
    abi: FACTORY_ABI,
    functionName: "isValidDAO",
    args: [daoAddress],
  });
}

export async function deactivateDaoOnFactory(
  wallet: PrivyEthereumWallet,
  daoAddress: Address
): Promise<Hex> {
  await wallet.switchChain(APP_CHAIN_ID);
...
export async function reactivateDaoOnFactory(
  wallet: PrivyEthereumWallet,
  daoAddress: Address
): Promise<Hex> {
  await wallet.switchChain(APP_CHAIN_ID);
