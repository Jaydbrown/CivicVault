...
const ProfileView: React.FC = () => {
  ...
  // Prefer MetaMask over embedded Privy wallet — same address regardless of login method.
  ...
  // Pre-fill notification email from linked Privy email
  useEffect(() => {
    if (linkedEmail) setNotifEmail(linkedEmail);
  }, [linkedEmail]);
  ...
  return (
    <div className="max-w-5xl mx-auto pb-6 lg:pb-12 animate-in fade-in duration-700 space-y-4 sm:space-y-6 w-full min-w-0">
      ...
      {/* Link MetaMask — shown when signed in with Gmail but no external wallet yet */}
      {embeddedOnly && (
        <Card className="p-5 border-emerald-100 bg-emerald-50/60">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            <button
              ...
              className="shrink-0 inline-flex items-center gap-2 px-4 py-2 bg-emerald-600 text-white rounded-xl text-sm font-semibold hover:bg-emerald-700 disabled:opacity-50 transition-colors"
            >
              <Link className="w-4 h-4" />
              {linkingWallet ? "Opening…" : "Link MetaMask"}
            </button>
          </div>
        </Card>
      )}

      {/* Notification email — shown when no email is linked yet (MetaMask users) */}
      {walletAddress && !linkedEmail && (
        <Card className="p-5 border-blue-100 bg-blue-50/40">
          ...
          <div className="flex gap-2">
            ...
            <button
              type="button"
              disabled={savingEmail || !notifEmail.trim()}
