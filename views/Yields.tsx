...
const YieldsView: React.FC = () => {
  ...
  if (loading) {
    return <div className="max-w-6xl mx-auto py-8 text-muted-foreground">Loading yield data from chain...</div>;
  }
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8 animate-in fade-in duration-500">
      <div className="flex justify-between items-end">
        <div>
          <h1 className="text-3xl font-bold text-foreground">Yield Earnings</h1>
          <p className="text-muted-foreground mt-2">Transparent neighborhood investment returns distributed on-chain.</p>
        </div>
        {canManageAnyDao && (
          <div className="bg-white/10 p-1 rounded-xl flex">
            <button
              onClick={() => setActiveTab('overview')}
              className={`px-4 py-2 rounded-lg text-xs font-bold transition-all ${activeTab === 'overview' ? 'bg-card backdrop-blur-md text-foreground shadow-sm' : 'text-muted-foreground hover:text-foreground/90'}`}
            >
              Overview
            </button>
            <button
              onClick={() => setActiveTab('distribute')}
              className={`px-4 py-2 rounded-lg text-xs font-bold transition-all ${activeTab === 'distribute' ? 'bg-card backdrop-blur-md text-foreground shadow-sm' : 'text-muted-foreground hover:text-foreground/90'}`}
            >
              Distribute Yields
            </button>
          </div>
      ...
      {activeTab === 'overview' ? (
        <div className="space-y-6">
          ...
          <Card className="p-0 overflow-hidden">
            <div className="p-6 border-b border-border/40">
              <h3 className="font-bold text-foreground">Investment Yield Rows</h3>
            </div>
            <div className="p-6 space-y-4">
              {rows.length === 0 ? (
                <p className="text-muted-foreground">No yield records yet.</p>
              ) : (
                rows.map((row) => (
                  <div key={`${row.daoAddress}-${row.investmentId}`} className="p-4 border border-border rounded-2xl">
                    <div className="flex justify-between items-start gap-4">
                      <div>
                        <p className="font-bold text-foreground">{row.investmentName}</p>
                        <p className="text-xs text-muted-foreground">{row.daoName}</p>
                        <div className="mt-1">
                      ...
                      <div className="text-right">
                        ...
                        <p className="text-xs text-muted-foreground">Total: {formatUsdcAmount(row.totalYield)}</p>
                      </div>
                    </div>
          ...
          <Card className="p-0 overflow-hidden">
            <div className="p-6 border-b border-border/40">
              <h3 className="font-bold text-foreground">Withdrawable Stakes</h3>
              <p className="text-xs text-muted-foreground mt-1">For investments marked as INCOMPLETE.</p>
            </div>
            <div className="p-6 space-y-4">
              {withdrawRows.length === 0 ? (
                <p className="text-muted-foreground">No withdrawable stake right now.</p>
              ) : (
                withdrawRows.map((row) => (
                  <div key={`withdraw-${row.daoAddress}-${row.investmentId}`} className="p-4 border border-border rounded-2xl">
          ...
          <Card className="p-6 space-y-3">
            <div className="flex items-center justify-between">
              <h3 className="font-bold text-foreground">Recent Yield Activity</h3>
            ...
            {rows.length === 0 ? (
              ...
              rows.slice(0, 6).map((row) => (
                <div key={`activity-${row.daoAddress}-${row.investmentId}`} className="p-3 border border-border rounded-xl">
                  <p className="text-sm font-bold text-foreground">{row.investmentName}</p>
                  <p className="text-xs text-muted-foreground">{row.daoName}</p>
                </div>
              ))
            )}
        ...
        <Card className="p-0 overflow-hidden">
          <div className="p-8 bg-white/5 border-b border-border/40 flex justify-between items-center">
            <div>
              <h3 className="text-xl font-bold text-foreground">Distribute Yields</h3>
              <p className="text-sm text-muted-foreground mt-1">Finance managers can deposit returns for each investment.</p>
            </div>
            <Coins className="w-10 h-10 text-slate-200" />
          </div>
          <div className="p-8 space-y-4">
            {rows.length === 0 ? (
              <p className="text-muted-foreground">No investments available.</p>
            ) : (
              rows.map((row) => (
                <div key={`${row.daoAddress}-${row.investmentId}`} className="p-4 border border-border rounded-xl flex justify-between items-center">
                  <div>
                    <p className="font-bold text-foreground">{row.investmentName}</p>
                    <p className="text-xs text-muted-foreground">{row.daoName}</p>
                  </div>
      ...
      >
        <div className="space-y-4">
          <div className="p-3 bg-white/5 rounded-xl">
            ...
            <p className="font-bold text-foreground">{selectedRow?.investmentName}</p>
          </div>
          <div className="space-y-2">
            <label className="text-sm font-bold text-foreground/90">Amount (USDC)</label>
            <input
              type="number"
              value={depositAmount}
          ...
          <div className="space-y-2">
            <label className="text-sm font-bold text-foreground/90">Expense Report CID</label>
            <input
              type="text"
              value={depositCid}
