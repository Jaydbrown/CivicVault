...
const CreateDAO: React.FC<CreateDAOProps> = ({ onComplete }) => {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('civicvault_create_progress');
    if (saved) setFormData(JSON.parse(saved));
  }, []);

  useEffect(() => {
    localStorage.setItem('civicvault_create_progress', JSON.stringify(formData));
  }, [formData]);
  ...
  const handleDeploy = async () => {
    ...
    try {
      ...
      if (result.daoAddress && typeof window !== 'undefined') {
        sessionStorage.setItem('civicvault_recent_created_dao', result.daoAddress);
      }
      setStep(5);
      localStorage.removeItem('civicvault_create_progress');
    } catch (error) {
