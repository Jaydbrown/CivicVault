...
const InvestmentListing: React.FC<{ onVote: (id: string) => void }> = ({ onVote }) => {
  ...
  if (loading) {
    return <div className="max-w-7xl mx-auto py-12 text-muted-foreground">Loading on-chain investments...</div>;
  }

  if (error) {
    return (
      <div className="max-w-7xl mx-auto py-12">
        <h2 className="text-xl font-bold text-foreground">Could not load investments</h2>
    ...
    <div className="max-w-7xl mx-auto space-y-8 animate-in fade-in duration-700">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
        <div>
          ...
          <p className="text-muted-foreground mt-2">Browse and vote on local neighborhood assets.</p>
        </div>
          ...
          {['All', 'Proposed', 'Active', 'Incomplete', 'Completed'].map(f => (
            <button
              ...
              className={`px-4 py-2 rounded-xl text-sm font-bold transition-all border ${
                filter === f ? 'bg-primary text-white border-slate-900' : 'bg-card backdrop-blur-md text-muted-foreground border-border hover:border-slate-300'
              }`}
            >
              {f}
            </button>
      ...
      {filtered.length === 0 ? (
        <div className="text-center py-20">
          ...
          <h3 className="text-xl font-bold text-foreground">No investments found</h3>
          <p className="text-muted-foreground mt-2">No matching proposals on-chain yet.</p>
        </div>
      ) : (
        <>
          ...
          {filtered.slice(0, visibleCount).map(inv => {
            ...
            return (
              <div
                ...
                className="bg-card backdrop-blur-md rounded-[2rem] border border-border overflow-hidden hover:shadow-xl hover:border-emerald-200 transition-all group flex flex-col"
              >
                <div className="h-44 relative overflow-hidden bg-white/10">
                  <img
                ...
                <div className="p-6 flex-grow flex flex-col">
                  <div className="flex gap-2 items-center mb-3">
                    <span className="text-[10px] font-bold text-muted-foreground uppercase bg-white/10 px-2 py-0.5 rounded">
                      {inv.daoName}
                    </span>
                  </div>
                  <h3 className="text-xl font-bold text-foreground group-hover:text-emerald-600 transition-colors mb-2">{inv.name}</h3>
                  ...
                  <div className="grid grid-cols-2 gap-4 mb-6 pt-4 border-t border-slate-50">
                    <div className="space-y-1">
                      <p className="text-[9px] font-bold text-muted-foreground uppercase tracking-widest">Raised</p>
                      <p className="text-sm font-bold text-foreground">{formatUsdcAmount(inv.upvotes)}</p>
                    </div>
                    <div className="space-y-1">
                      <p className="text-[9px] font-bold text-muted-foreground uppercase tracking-widest">Yield</p>
                  ...
                  <div className="mt-auto flex gap-3">
                    <button
                      ...
                      className="flex-grow py-3 bg-card backdrop-blur-md border border-border text-foreground/90 rounded-xl text-xs font-bold hover:bg-white/5 transition-colors"
                    >
                      Details
                    </button>
                    {status === 'Proposed' && (
                      <button
                        ...
                        className="flex-grow py-3 bg-primary text-white rounded-xl text-xs font-bold hover:shadow-lg transition-shadow flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
      ...
      {selectedInvestment && modalRoot && createPortal(
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 sm:p-6">
          <div className="absolute inset-0 bg-black/60/60 backdrop-blur-sm" onClick={() => setSelectedInvestment(null)} />
          <div className="relative w-full max-w-2xl bg-card backdrop-blur-md rounded-[2rem] shadow-2xl p-8">
            <button
              onClick={() => setSelectedInvestment(null)}
              className="absolute top-5 right-5 p-2 rounded-full bg-white/10 hover:bg-slate-200"
            >
              <X className="w-4 h-4" />
            </button>
            <h2 className="text-2xl font-bold text-foreground mb-2">{selectedInvestment.name}</h2>
            <p className="text-muted-foreground mb-6">{selectedInvestment.daoName}</p>
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div className="p-4 rounded-xl bg-white/5">
                <p className="text-muted-foreground text-xs uppercase">Investment ID</p>
              ...
              <div className="p-4 rounded-xl bg-white/5">
                <p className="text-muted-foreground text-xs uppercase">Goal</p>
                <p className="font-bold">{formatUsdcAmount(selectedInvestment.fundNeeded)}</p>
              </div>
              <div className="p-4 rounded-xl bg-white/5">
                <p className="text-muted-foreground text-xs uppercase">Current Upvotes</p>
      ...
      {extendModal && modalRoot && createPortal(
        <div className="fixed inset-0 z-[70] flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-black/60/60 backdrop-blur-sm" onClick={() => setExtendModal(null)} />
          <div className="relative w-full max-w-md bg-card backdrop-blur-md rounded-2xl shadow-2xl p-6 space-y-4">
            <h3 className="text-lg font-bold text-foreground">Extend Deadline</h3>
            <p className="text-sm text-muted-foreground">{extendModal.name}</p>
            <div className="space-y-2">
              <label className="text-sm font-bold text-foreground/90">Additional Days (1-90)</label>
              <input
                type="number"
                min="1"
                max="90"
                ...
                className="w-full p-3 bg-white/5 border border-border rounded-xl outline-none"
              />
            </div>
            <div className="flex justify-end gap-2 pt-2">
              <button
                onClick={() => setExtendModal(null)}
                className="px-4 py-2 rounded-lg border border-border text-sm font-bold"
              >
                Cancel
              </button>
              <button
      ...
      {createModalOpen && modalRoot && createPortal(
        <div className="fixed inset-0 z-[70] flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-black/60/60 backdrop-blur-sm" onClick={() => setCreateModalOpen(false)} />
          <div className="relative w-full max-w-lg bg-card backdrop-blur-md rounded-2xl shadow-2xl p-6 space-y-4">
            <h3 className="text-lg font-bold text-foreground">Create Investment Proposal (On-chain)</h3>
            <div>
              <label className="block text-xs font-bold text-muted-foreground mb-1">DAO</label>
              <select
                value={createDaoAddress}
                onChange={(e) => setCreateDaoAddress(e.target.value)}
                className="w-full p-2.5 bg-white/5 border border-border rounded-lg text-sm"
              >
                {managedDaos.map((dao) => (
            ...
            <div>
              <label className="block text-xs font-bold text-muted-foreground mb-1">Deadline (Days)</label>
              <input
                type="number"
